...
def classify_columns_node(state: WorkflowState, llm: BaseChatModel) -> Dict[str, Any]:
    ...
    logger.debug(
        f"df_json type: {type(state.get('df_json'))}, length: {len(state.get('df_json', '')) if state.get('df_json') else 0}"
    )
    ...
    try:
        ...
        logger.debug(
            f"Classification result keys: {list(result.keys()) if isinstance(result, dict) else 'not a dict'}"
        )
        ...
        try:
            ...
            correlation_data = compute_correlation_matrix.invoke(
                {"df_json": state["df_json"], "columns": None}
            )
            logger.debug(
                f"Correlation data computed (length: {len(correlation_data) if correlation_data else 0})"
            )
        except Exception as e:
...
def build_final_config_node(state: WorkflowState) -> Dict[str, Any]:
    ...
    final_config = {
        ...
        "location_settings": {"max_distance_km": int(location_settings.get("max_distance_km", 50))},
        "model": {
            "targets": classification.get("targets", []),
...
def create_workflow_graph(llm: BaseChatModel) -> StateGraph:
    ...
    workflow.add_conditional_edges(
        ...
        {"encode_features": "encode_features", "await_classification": "await_classification"},
    )
    ...
    workflow.add_conditional_edges(
        ...
        {"configure_model": "configure_model", "await_encoding": "await_encoding"},
    )
...
def compile_workflow(llm: BaseChatModel, checkpointer: Optional[MemorySaver] = None) -> Any:
    ...
    return workflow.compile(
        checkpointer=checkpointer, interrupt_before=["await_classification", "await_encoding"]
    )
...
class ConfigWorkflow:
    ...
    def confirm_classification(
        ...
        if modifications:
            ...
            if column_types:
                location_columns = [
                    col for col, col_type in column_types.items() if col_type == "location"
                ]
