...
def mock_analytics_service():
    with patch("src.app.app.AnalyticsService") as MockAn:
        ...
        mock_instance.get_feature_importance.return_value = pd.DataFrame({
            "Feature": ["Level", "Location", "YearsOfExperience"],
            "Gain": [10.0, 8.0, 5.0]
        })
...
def test_model_analysis_section(mock_streamlit, mock_registry, mock_forecaster, mock_analytics_service):
...
def test_model_analysis_feature_importance(mock_streamlit, mock_registry, mock_forecaster, mock_analytics_service):
