...
class TestComputeCorrelationMatrix(unittest.TestCase):
    def test_basic_correlation(self):
        ...
        ab_corr = next(
            c
            for c in result_dict["correlations"]
            if (c["column_1"] == "A" and c["column_2"] == "B")
            or (c["column_1"] == "B" and c["column_2"] == "A")
        )
        self.assertAlmostEqual(ab_corr["correlation"], 1.0, places=4)

    def test_specific_columns(self):
        df = pd.DataFrame({"A": [1, 2, 3], "B": [2, 4, 6], "C": [1, 1, 1], "D": "text"})

        result = compute_correlation_matrix.invoke({"df_json": df.to_json(), "columns": "A, B"})
        result_dict = json.loads(result)
    ...
    def test_column_name_filtering_whitespace(self):
        ...
        result = compute_correlation_matrix.invoke(
            {"df_json": df.to_json(), "columns": " A ,  B  , C "}
        )
        result_dict = json.loads(result)
    ...
    def test_column_name_special_chars(self):
        ...
        df = pd.DataFrame({"col-name": [1, 2, 3], "col_name": [2, 4, 6], "col.name": [3, 6, 9]})

        result = compute_correlation_matrix.invoke(
            {"df_json": df.to_json(), "columns": "col-name, col_name"}
        )
        result_dict = json.loads(result)
...
class TestGetColumnStatistics(unittest.TestCase):
    ...
    def test_numeric_column(self):
        ...
        result = get_column_statistics.invoke({"df_json": df.to_json(), "column": "values"})
        result_dict = json.loads(result)
    ...
    def test_categorical_column(self):
        ...
        result = get_column_statistics.invoke({"df_json": df.to_json(), "column": "category"})
        result_dict = json.loads(result)
    ...
    def test_missing_column(self):
        ...
        result = get_column_statistics.invoke({"df_json": df.to_json(), "column": "NonExistent"})
        result_dict = json.loads(result)
    ...
    def test_all_null_values(self):
        ...
        result = get_column_statistics.invoke({"df_json": df.to_json(), "column": "null_col"})
        result_dict = json.loads(result)
    ...
    def test_datetime_column(self):
        ...
        df = pd.DataFrame({"dates": pd.date_range("2023-01-01", periods=5, freq="D")})

        result = get_column_statistics.invoke({"df_json": df.to_json(), "column": "dates"})
        result_dict = json.loads(result)
    ...
    def test_boolean_column(self):
        ...
        df = pd.DataFrame({"flags": [True, False, True, False, True]})

        result = get_column_statistics.invoke({"df_json": df.to_json(), "column": "flags"})
        result_dict = json.loads(result)
    ...
    def test_boolean_column_edge_cases(self):
        ...
        df = pd.DataFrame({"binary": [0, 1, 0, 1, 0]})

        result = get_column_statistics.invoke({"df_json": df.to_json(), "column": "binary"})
        result_dict = json.loads(result)
    ...
    def test_large_dataset(self):
        ...
        np.random.seed(42)
        df = pd.DataFrame({"large_col": np.random.randn(10000)})

        result = get_column_statistics.invoke({"df_json": df.to_json(), "column": "large_col"})
        result_dict = json.loads(result)
...
class TestGetUniqueValueCounts(unittest.TestCase):
    ...
    def test_basic_counts(self):
        ...
        df = pd.DataFrame({"status": ["Active", "Active", "Inactive", "Active", "Pending"]})

        result = get_unique_value_counts.invoke({"df_json": df.to_json(), "column": "status"})
        result_dict = json.loads(result)

        self.assertEqual(result_dict["total_unique_values"], 3)

        # Check Active count
        active_count = next(v for v in result_dict["value_counts"] if v["value"] == "Active")
    ...
    def test_limit(self):
        ...
        result = get_unique_value_counts.invoke(
            {"df_json": df.to_json(), "column": "id", "limit": 5}
        )
        result_dict = json.loads(result)
    ...
    def test_all_null_values(self):
        ...
        result = get_unique_value_counts.invoke({"df_json": df.to_json(), "column": "null_col"})
        result_dict = json.loads(result)
    ...
    def test_high_cardinality(self):
        ...
        df = pd.DataFrame({"high_card": [f"value_{i}" for i in range(1500)]})

        result = get_unique_value_counts.invoke(
            {"df_json": df.to_json(), "column": "high_card", "limit": 20}
        )
        result_dict = json.loads(result)
    ...
    def test_limit_zero(self):
        ...
        result = get_unique_value_counts.invoke(
            {"df_json": df.to_json(), "column": "values", "limit": 0}
        )
        result_dict = json.loads(result)
    ...
    def test_percentage_with_nulls(self):
        ...
        df = pd.DataFrame({"mixed": ["A", "A", "B", None, None, "C"]})

        result = get_unique_value_counts.invoke({"df_json": df.to_json(), "column": "mixed"})
        result_dict = json.loads(result)
...
class TestDetectOrdinalPatterns(unittest.TestCase):
    ...
    def test_numeric_pattern(self):
        ...
        df = pd.DataFrame({"level": ["L1", "L2", "L3", "L4", "L5"]})

        result = detect_ordinal_patterns.invoke({"df_json": df.to_json(), "column": "level"})
        result_dict = json.loads(result)
    ...
    def test_keyword_pattern(self):
        ...
        df = pd.DataFrame({"seniority": ["Junior Developer", "Senior Developer", "Lead Developer"]})

        result = detect_ordinal_patterns.invoke({"df_json": df.to_json(), "column": "seniority"})
        result_dict = json.loads(result)
    ...
    def test_non_ordinal(self):
        ...
        result = detect_ordinal_patterns.invoke({"df_json": df.to_json(), "column": "color"})
        result_dict = json.loads(result)
    ...
    def test_roman_numerals(self):
        ...
        df = pd.DataFrame({"tier": ["Tier I", "Tier II", "Tier III", "Tier IV", "Tier V"]})

        result = detect_ordinal_patterns.invoke({"df_json": df.to_json(), "column": "tier"})
        result_dict = json.loads(result)
    ...
    def test_roman_numerals_all(self):
        ...
        df = pd.DataFrame({"levels": ["I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX", "X"]})

        result = detect_ordinal_patterns.invoke({"df_json": df.to_json(), "column": "levels"})
        result_dict = json.loads(result)
    ...
    def test_mixed_patterns(self):
        ...
        df = pd.DataFrame({"mixed": ["Level 1", "Level 2", "Senior", "Junior", "Level 3"]})

        result = detect_ordinal_patterns.invoke({"df_json": df.to_json(), "column": "mixed"})
        result_dict = json.loads(result)
    ...
    def test_case_insensitive_keywords(self):
        ...
        df = pd.DataFrame({"roles": ["JUNIOR", "SENIOR", "LEAD", "junior", "senior"]})

        result = detect_ordinal_patterns.invoke({"df_json": df.to_json(), "column": "roles"})
        result_dict = json.loads(result)
    ...
    def test_no_clear_pattern(self):
        ...
        df = pd.DataFrame({"random": ["Apple", "Banana", "Cherry", "Date", "Elderberry"]})

        result = detect_ordinal_patterns.invoke({"df_json": df.to_json(), "column": "random"})
        result_dict = json.loads(result)
    ...
    def test_very_long_category_names(self):
        ...
        result = detect_ordinal_patterns.invoke({"df_json": df.to_json(), "column": "long_names"})
        result_dict = json.loads(result)
...
class TestDetectColumnDtype(unittest.TestCase):
    ...
    def test_numeric_continuous(self):
        ...
        df = pd.DataFrame({"price": [10.5, 20.3, 15.7, 30.2]})

        result = detect_column_dtype.invoke({"df_json": df.to_json(), "column": "price"})
        result_dict = json.loads(result)
    ...
    def test_categorical(self):
        ...
        result = detect_column_dtype.invoke({"df_json": df.to_json(), "column": "category"})
        result_dict = json.loads(result)
    ...
    def test_identifier(self):
        ...
        df = pd.DataFrame({"user_id": list(range(100))})

        result = detect_column_dtype.invoke({"df_json": df.to_json(), "column": "user_id"})
        result_dict = json.loads(result)
    ...
    def test_boolean(self):
        ...
        result = detect_column_dtype.invoke({"df_json": df.to_json(), "column": "is_active"})
        result_dict = json.loads(result)
    ...
    def test_datetime_strings(self):
        ...
        result = detect_column_dtype.invoke({"df_json": df.to_json(), "column": "dates"})
        result_dict = json.loads(result)
    ...
    def test_numeric_strings(self):
        ...
        df = pd.DataFrame({"numeric_str": ["123", "456", "789", "012"]})

        result = detect_column_dtype.invoke({"df_json": df.to_json(), "column": "numeric_str"})
        result_dict = json.loads(result)
    ...
    def test_high_cardinality_categorical_vs_text(self):
        ...
        df = pd.DataFrame({"high_card": [f"cat_{i % 10}" for i in range(100)]})

        result = detect_column_dtype.invoke({"df_json": df.to_json(), "column": "high_card"})
        result_dict = json.loads(result)
    ...
    def test_identifier_detection_patterns(self):
        ...
        for col_name, data in test_cases:
            ...
            result = detect_column_dtype.invoke({"df_json": df.to_json(), "column": col_name})
            result_dict = json.loads(result)
    ...
    def test_boolean_like_values(self):
        ...
        for data, expected_type in test_cases:
            ...
            result = detect_column_dtype.invoke({"df_json": df.to_json(), "column": "bool_col"})
            result_dict = json.loads(result)

            # May be boolean or numeric_discrete depending on dtype
            self.assertIn(
                result_dict["semantic_type"], [expected_type, "boolean", "numeric_discrete"]
            )
...
class TestEscapedJsonHandling(unittest.TestCase):
    ...
    def test_compute_correlation_matrix_with_escaped_json(self):
        ...
        result = compute_correlation_matrix.invoke({"df_json": escaped_json, "columns": None})
        result_dict = json.loads(result)
    ...
    def test_get_column_statistics_with_escaped_json(self):
        ...
        df = pd.DataFrame({"Level": ["E6", "E6", "E3"], "Salary": [100000, 150000, 120000]})
        normal_json = df.to_json()
        escaped_json = json.dumps(normal_json)

        result = get_column_statistics.invoke({"df_json": escaped_json, "column": "Salary"})
        result_dict = json.loads(result)
    ...
    def test_detect_column_dtype_with_escaped_json(self):
        ...
        df = pd.DataFrame({"Level": {"0": "E6", "1": "E6", "2": "E3"}})
        normal_json = df.to_json()
        escaped_json = json.dumps(normal_json)

        result = detect_column_dtype.invoke({"df_json": escaped_json, "column": "Level"})
        result_dict = json.loads(result)
    ...
    def test_get_unique_value_counts_with_escaped_json(self):
        ...
        result = get_unique_value_counts.invoke({"df_json": escaped_json, "column": "Category"})
        result_dict = json.loads(result)
    ...
    def test_detect_ordinal_patterns_with_escaped_json(self):
        ...
        df = pd.DataFrame({"Level": ["L1", "L2", "L3", "L4"]})
        normal_json = df.to_json()
        escaped_json = json.dumps(normal_json)

        result = detect_ordinal_patterns.invoke({"df_json": escaped_json, "column": "Level"})
        result_dict = json.loads(result)
    ...
    def test_double_quoted_escaped_json(self):
        ...
        result = compute_correlation_matrix.invoke({"df_json": double_quoted, "columns": None})
        result_dict = json.loads(result)
