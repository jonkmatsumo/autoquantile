...
def test_monotonicity_level(trained_model):
    ...
    base_input = {"Location": "New York", "YearsOfExperience": 5, "YearsAtCompany": 2}
...
def test_monotonicity_yoe(trained_model):
    ...
    base_input = {"Level": "E5", "Location": "New York", "YearsAtCompany": 2}
...
def test_location_impact(trained_model):
    ...
    assert isinstance(pred_ny, (int, float, np.number)) and isinstance(
        pred_austin, (int, float, np.number)
    ), f"Predictions should be numeric, got NY={type(pred_ny)}, Austin={type(pred_austin)}"
    if pred_ny <= pred_austin:
        print(
            f"Warning: NY prediction ({pred_ny}) not higher than Austin ({pred_austin}) - model may need more training data"
        )
...
class TestConfigHyperparams(unittest.TestCase):
    def setUp(self):
        ...
        self.custom_config = {
            ...
            "model": {
                ...
                "hyperparameters": {
                    "training": {"objective": "reg:quantileerror", "max_depth": 7, "eta": 0.01},
                    "cv": {"num_boost_round": 10, "nfold": 2},
                },
            },
            "feature_engineering": {},
        }
    ...
    def test_analyze_cv_results_static(self):
        ...
        cv_df = pd.DataFrame(
            {"test-quantile-mean": [0.5, 0.4, 0.45, 0.6], "other-metric": [1, 2, 3, 4]}
        )
...
class TestOutlierDetection:
    ...
    def test_remove_outliers_iqr(self, forecaster):
        ...
        # Outlier: 1000
        data = pd.DataFrame(
            {"Salary": [100, 102, 98, 101, 99, 1000, 100, 100], "Date": ["2023-01-01"] * 8}
        )
    ...
    def test_train_calls_remove_outliers(self, forecaster):
        ...
        forecaster.remove_outliers = MagicMock(
            return_value=(pd.DataFrame({"Salary": [100], "Date": ["2023-01-01"]}), 1)
        )
...
class TestPreprocess(unittest.TestCase):
    ...
    def setUp(self):
        self.config = {
            "mappings": {"levels": {"E3": 0, "E4": 1}, "location_targets": {"New York": 1}},
            "location_settings": {"max_distance_km": 50},
            "model": {
                "targets": ["BaseSalary"],
                "quantiles": [0.5],
    ...
    def test_preprocess_missing_features(self):
        ...
        df = pd.DataFrame({"Level": ["E3"], "Location": ["New York"]})
...
class TestPredict(unittest.TestCase):
    ...
    def test_predict_multiple_targets(self, mock_dmatrix):
        ...
        mock_model = MagicMock()
        mock_model.predict.return_value = np.array([100000])
        forecaster.models = {"BaseSalary_p50": mock_model, "TotalComp_p50": mock_model}
    ...
    def test_preprocess_with_metro_population_encoding(self, mock_geo_mapper):
        ...
        config = {
            ...
            "model": {
                "targets": ["BaseSalary"],
                "quantiles": [0.5],
                "features": [{"name": "Location_MetroPopulation", "monotone_constraint": 0}],
                "hyperparameters": {"training": {}, "cv": {}},
            },
            "feature_engineering": {},
    ...
    def test_preprocess_with_normalize_recent_date_encoding(self):
        ...
        config = {
            ...
            "model": {
                "targets": ["BaseSalary"],
                "quantiles": [0.5],
                "features": [{"name": "Date_Normalized", "monotone_constraint": 0}],
                "hyperparameters": {"training": {}, "cv": {}},
            },
            "feature_engineering": {},
    ...
    def test_preprocess_with_weight_recent_date_encoding(self, mock_geo_mapper):
        ...
        config = {
            ...
            "model": {
                "targets": ["BaseSalary"],
                "quantiles": [0.5],
                "features": [{"name": "YearsOfExperience", "monotone_constraint": 1}],
                "sample_weight_k": 1.0,
                "hyperparameters": {"training": {}, "cv": {}},
            },
            "feature_engineering": {},
