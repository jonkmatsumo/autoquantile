...
def test_sample_weighter_dataframe_with_date_col():
    ...
    df = pd.DataFrame({"Date": ["2023-01-01", "2022-01-01", "2021-01-01"], "Other": [1, 2, 3]})
...
def test_sample_weighter_dataframe_missing_date_col():
    ...
    df = pd.DataFrame({"Date": ["2023-01-01", "2022-01-01"], "Other": [1, 2]})
...
def test_date_normalizer_nat_handling():
    ...
    dates = pd.Series([pd.Timestamp("2020-01-01"), pd.NaT, pd.Timestamp("2022-01-01")])
...
def test_date_normalizer_dataframe_input():
    ...
    df = pd.DataFrame({"Date": [pd.Timestamp("2020-01-01"), pd.Timestamp("2022-01-01")]})
